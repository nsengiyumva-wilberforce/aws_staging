...
class Form extends CI_Controller {
	...
	public function add_mobile_user()
	{
		...
		// Set region_id properly instead of relying on API default
		...
		if ($result && isset($result->status) && ($result->status == 200 || $result->status == 201)) {
			...
			} elseif ($result && isset($result->messages)) {
				// Handle both array and object message formats
				if (is_array($result->messages)) {
					$error_message = implode(', ', $result->messages);
				} else {
					$error_message = implode(', ', (array)$result->messages);
				}
			}
